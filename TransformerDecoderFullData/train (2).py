import TransformerDecoderFullData.data as dataset
from torch.utils.data import DataLoader
import TransformerDecoderFullData.model as model
import torch
import numpy as np
...
class DecoderTrainer:
    ...
    def computeLoss(self, true, prediction):
        ...
        # breakpoint()
        loss = torch.mul(
            100, torch.mean((true[:, 0, :, 2:4] - prediction[:, 0, :, 2:4]) ** 2)
    ...
    def analyzeSpecific(self, true, prediction, indices):
        losses = torch.mul(
            100, torch.mean((true[:, 0, :, :2] - prediction[:, 0, :, :2]) ** 2, dim=2)
        )
        breakpoint()
