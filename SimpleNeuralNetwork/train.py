import SimpleNeuralNetwork.data as dataset
from torch.utils.data import DataLoader
import SimpleNeuralNetwork.model as model
import torch
import numpy as np
...
class SimpleNNTrainer:
    ...
    def train(self):
        ...
        for epoch in range(self.config["EPOCHS"]):
            ...
            if self.config.get("ANALYZE"):
                unnormalized_score_tr = self.analyze(self.train_dataloader)
                print(f"Unnormalized score (train) {unnormalized_score_tr}")

                unnormalized_score_te = self.analyze(self.test_dataloader)
                print(f"Unnormalized score (test) {unnormalized_score_te}")
    ...
    def analyze(self, dataloader: DataLoader):
        ...
        for batch, (X, y, org_indices) in enumerate(dataloader):
            ...
            true_unnormalized = dataloader.dataset.unnormalizeData(
                y.detach().numpy(), org_indices
            )
            pred_unnormalized = dataloader.dataset.unnormalizeData(
