...
def createOriginalSpacePredictions(Y, org_data):
    orig_space_pos = (Y[:, :, :, :2] / POS_DIFF_MULTIPLIER) + org_data[:, 0:1, 49:50, :2]
...
def main():
    ...
    os.makedirs(
        os.path.join(DATA_DIR, "IntermediateData", "TransformerDecoderVel"),
        exist_ok=True,
    )
    np.savez(
        os.path.join(
            DATA_DIR, "IntermediateData", "TransformerDecoderVel", "train_mini.npz"
        ),
        data=train_aux,
